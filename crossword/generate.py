...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        for x_domain in self.domains[x].copy():
            ...
            for y_domain in self.domains[y].copy():
                ...
                # Checked all of Y's domain and did not break out of the loop
                if counter == y_num:
                    revised = True
                    self.domains[x].remove(x_domain)

                # Does not cause conflict -> we can move to the next domain of X
                if x_domain[x_overlap] == y_domain[y_overlap]:
                    break
                else:
                    counter += 1
    ...
    def assignment_complete(self, assignment):
        ...
        """
        for variable in self.crossword.variables:
            # If one variable is not in assignment, then the crossword is missing a variable.
            if variable not in assignment.keys():
                return False
            # If the variable does not have a proper value
            ...
            for variable2, word2 in assignment.items():
                ...
                # No conflicts between neighboring variables.
                    if self.crossword.overlaps[variable1, variable2]:
                        (var1_overlap, var2_overlap) = self.crossword.overlaps[variable1, variable2]
            ...
            for neighbor in self.crossword.neighbors(var):
                if neighbor not in assignment.keys():
                    ...
                    for neighbor_domain in self.domains[neighbor]:
                        if var_domain[var_index] != neighbor_domain[neighbor_index]:
                            counter += 1
                    
            result[var_domain] = counter
            counter = 0
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for variable in self.crossword.variables:
            if variable not in assignment:
                # If there is a potential variable store it with it's minimum remaining value heuristic and degree heuristic
                result.append([variable, len(self.domains[variable]), len(self.crossword.neighbors(variable))])
