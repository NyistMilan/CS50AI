...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                # Ignore if the cell has been revealed
                if (i, j) in self.moves_made:
                    continue

                # If cell is a mine, decrease count
                if (i, j) in self.mines:
                    count -= 1
                    continue

                # Ignore if we know the cell cannot be a mine
                if (i, j) in self.safes:
                    continue

                # Update neighbours if cell in bounds
                if 0 <= i < self.height and 0 <= j < self.width:
                    neighbours.add((i, j))
    ...
    def make_safe_move(self):
        ...
        if safeMoves:
            return random.choice(tuple(safeMoves))
